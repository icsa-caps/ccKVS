...
//    printf("Remote clients buffer size %d\n", remote_buf_size);
    ...
    struct hrd_ctrl_blk *cb = hrd_ctrl_blk_init(clt_gid,	/* local_hid */
                                                ...
                                                CLIENT_UD_QPS, remote_buf_size + EC_CLT_BUF_SIZE,	/* num_dgram_qps, dgram_buf_size */
...
//    printf("dgram pointer %llu, incoming req ptr %llu \n", cb->dgram_buf, incoming_reqs);
    ...
    struct ibv_send_wr rem_send_wr[WINDOW_SIZE], coh_send_wr[MESSAGES_IN_BCAST_BATCH],
            credit_send_wr[EC_MAX_CREDIT_WRS], *bad_send_wr;
    struct ibv_sge rem_send_sgl[WINDOW_SIZE], coh_send_sgl[MAX_BCAST_BATCH], credit_send_sgl;
    struct ibv_wc wc[MAX_REMOTE_RECV_WCS], signal_send_wc, credit_wc[EC_MAX_CREDIT_WRS], coh_wc[EC_MAX_COH_RECEIVES];
    struct ibv_recv_wr rem_recv_wr[WINDOW_SIZE], coh_recv_wr[EC_MAX_COH_RECEIVES],
            credit_recv_wr[EC_MAX_CREDIT_RECVS], *bad_recv_wr;
    struct ibv_sge rem_recv_sgl, coh_recv_sgl[EC_MAX_COH_RECEIVES], credit_recv_sgl;
    ...
    uint8_t rm_id = 0, per_worker_outstanding[WORKER_NUM] = {0}, credits[EC_VIRTUAL_CHANNELS][MACHINE_NUM],
    ...
    ---------------------------------------------------------------------------*/
    /* Remote Requests */
    for(i = 0; i < WINDOW_SIZE; i++) // TODO take care that coalesced worker responses do not overwrite coherence
    ...
    while (1) {
        ...
        coh_i = 0;
        if (WRITE_RATIO > 0 && DISABLE_CACHE == 0) {
            poll_coherence_EC(&coh_i, incoming_reqs, &pull_ptr, update_ops, update_resp, clt_gid, local_client_id);
            ...
            if (coh_i > 0) {
                ...
                credit_wr_i =  create_credits_EC(coh_i, coh_wc, broadcasts_seen, local_client_id,
                ...
                if (credit_wr_i > 0)
                    send_credits(credit_wr_i, coh_recv_sgl, cb, &push_ptr, coh_recv_wr, coh_recv_qp,
                                 credit_send_wr,  (uint16_t)EC_CREDITS_IN_MESSAGE, (uint32_t)EC_CLT_BUF_SLOTS,
                                 (void*)incoming_reqs);
                // push the new coherence messages to the CACHES
                cache_batch_op_ec_with_cache_op(coh_i, local_client_id, &update_ops, update_resp);
            }
        }
        /* ---------------------------------------------------------------------------
        ------------------------------PROBE THE CACHE--------------------------------------
        ...
        ---------------------------------------------------------------------------*/
        if (WRITE_RATIO > 0 && DISABLE_CACHE == 0) {
            perform_broadcasts_EC(ops, credits, cb, credit_wc,
