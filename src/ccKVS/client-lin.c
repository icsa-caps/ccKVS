...
void *run_client(void *arg)
{
	...
	if (ENABLE_MULTICAST == 1 && local_client_id == 0) {
		red_printf("MULTICAST IS NOT WORKING IN LIN, PLEASE DISABLE IT\n");
		// TODO to fix it we must post receives seperately for acks and multicasts
		assert(false);
	}
	int protocol = LINEARIZABILITY;
	...
	struct hrd_ctrl_blk *cb = hrd_ctrl_blk_init(clt_gid,	/* local_hid */
												...
												CLIENT_UD_QPS, remote_buf_size + LIN_CLT_BUF_SIZE,	/* num_dgram_qps, dgram_buf_size */
	...
	while(1) {
		...
		if (WRITE_RATIO > 0 && DISABLE_CACHE == 0) {
			poll_coherence_LIN(&update_ops_i, incoming_reqs, &pull_ptr, update_ops, clt_gid, local_client_id,
		...
		if (WRITE_RATIO > 0 && DISABLE_CACHE == 0) {
			...
			if (update_ops_i > 0) {
				cache_batch_op_lin_non_stalling_sessions_with_cache_op(update_ops_i, local_client_id, &update_ops,
																	   update_resp);
		...
		if (WRITE_RATIO > 0 && DISABLE_CACHE == 0) {
			if (inv_ops_i > 0) {
				...
				cache_batch_op_lin_non_stalling_sessions_with_small_cache_op(inv_ops_i, local_client_id, &inv_ops,
																			 inv_resp);
		...
		if (WRITE_RATIO > 0 && DISABLE_CACHE == 0) {
			...
			credit_wr_i = forge_credits_LIN(coh_message_count, acks_seen, invs_seen, upds_seen, local_client_id,
											credit_wr, &credit_tx,
		...
		------------------------------DEBUG--------------------------------
		---------------------------------------------------------------------------*/
		if (!DISABLE_CACHE && STALLING_DEBUG_LIN)
			debug_stalling_LIN(stalled_ops_i, &stalled_counter, &stalled, local_client_id);
	}
	return NULL;
}
