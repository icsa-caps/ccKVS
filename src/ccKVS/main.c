...
int main(int argc, char *argv[])
{
  ...
  if (HERD_GET_REQ_SIZE != (KEY_SIZE + 1))
	...
	if (DISABLE_CACHE == 0) {
		// assert(EC_CLIENT_SEND_BR_Q_DEPTH > EC_MAX_CREDIT_WRS); // EC credits
		// assert(SC_CLIENT_SEND_BR_Q_DEPTH > MAX_CREDIT_WRS); // SC credits
		// assert(EC_CLIENT_SEND_BR_Q_DEPTH > EC_MAX_COH_MESSAGES); // EC coherence messages (i.e. broadcasts)
		// assert(SC_CLIENT_SEND_BR_Q_DEPTH > MAX_COH_MESSAGES); // SC coherence messages
	}
	...
	if (DISABLE_CACHE == 0) {
		assert(EC_CLIENT_RECV_CR_Q_DEPTH > EC_MAX_CREDIT_RECVS); //ec credit recvs
		assert(SC_CLIENT_RECV_CR_Q_DEPTH > MAX_CREDIT_RECVS); //sc credit recvs
		assert(EC_CLIENT_RECV_BR_Q_DEPTH > EC_MAX_COH_RECEIVES); // ec coh recvs
		assert(SC_CLIENT_RECV_BR_Q_DEPTH > MAX_COH_RECEIVES);  // sc coh recvs
		assert(ISSUERS_PER_MACHINE < TOTAL_CORES);
	}
	...
	assert(EC_CREDITS % EC_CREDIT_DIVIDER == 0); // the division should better be perfect
...
//  if (ENABLE_WORKER_COALESCING == 0) assert(CLIENT_REMOTE_BUFF_SIZE == UD_REQ_SIZE);
  ...
  assert(CACHE_BATCH_OP_SC == cache_batch_op_sc_non_stalling_sessions);
	...
	thread_arr = malloc((CLIENTS_PER_MACHINE + WORKERS_PER_MACHINE + ISSUERS_PER_MACHINE + 1) * sizeof(pthread_t));
  ...
  latency_count.total_measurements = 0;
	...
	if (ENABLE_ISSUERS) {
		//Create request issuer threads
		int issuer_counter = 0;
		for(i = 0; i < TOTAL_CORES; i++)
			if (occupied_cores[i] == 0){
				if(issuer_counter == ISSUERS_PER_MACHINE) break;
				CPU_ZERO(&cpus_c);
				CPU_SET(i, &cpus_c);
				issuer_arr[issuer_counter].id = issuer_counter++;
				cyan_printf("Creating issuer thread %d at core %d \n", issuer_counter, i);
				pthread_attr_setaffinity_np(&attr, sizeof(cpu_set_t), &cpus_c);
				pthread_create(&thread_arr[i + CLIENTS_PER_MACHINE + WORKERS_PER_MACHINE], &attr, run_issuer, &issuer_arr[issuer_counter]);// change NULL here to &attr to get the thread affinity
			}
		if(issuer_counter != ISSUERS_PER_MACHINE)
			red_printf("Only %d out of %d issuer threads were created. \n", issuer_counter, ISSUERS_PER_MACHINE);
	}
	...
	for(i = 0; i < CLIENTS_PER_MACHINE + WORKERS_PER_MACHINE + ISSUERS_PER_MACHINE + 1; i++) {
